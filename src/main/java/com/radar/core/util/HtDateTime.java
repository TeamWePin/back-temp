...
import static com.radar.core.exception.RadarServiceStatusCode.ERROR_SYSTEM_EXCEPTION;
 ...
 *  HtDateTime.createDateTimeFrom(...).addYears(1).setDay(1).get("yyyyMMdd HH:mm:ss");
 */
@Slf4j
@JsonSerialize(using = HtDateTimeUtil.HtDateTimeSerializer.class)
@JsonDeserialize(using = HtDateTimeUtil.HtDateTimeDeserializer.class)
public class HtDateTime implements Cloneable, Serializable, Comparable<HtDateTime> {
    ...
    private HtDateTime setZone(ZoneId zone) {
        this.zone = zone;
        return this;
    }
    private HtDateTime setLocalDateTime(LocalDateTime localDateTime) {
     ...
     */
    private HtDateTime(ZoneId zone, LocalDateTime localDateTime) {
        this.zone = zone;
        this.localDateTime = localDateTime;
    }
    ...
    public static class Builder {
        ...
        public HtDateTime build() {
            return new HtDateTime(zone, localDateTime);
        }
    }
     ...
     */
    @Override
    public HtDateTime clone() {
        try {
            return (HtDateTime) super.clone();
        } catch (CloneNotSupportedException e) {
            // Cloneable 인터페이스를 구현받았고, Object 클래스를 바로 상속받기에 문제는 없지만
            // 나중에 구조가 바뀌게 된다면 에러가 발생할 수 있으므로, 주의바람
            logger.error("Can't clone HtDateTime object!", e);
     ...
     */
    public boolean isEqual(HtDateTime target) {
        return this.localDateTime.isEqual(target.localDateTime);
    }

    // yyyy-MM-dd 비교, 즉 연월일이 동일한지 확인
    public boolean isDateEqual(HtDateTime target) {
        return (this.getYear().equals(target.getYear()) && this.getMonth().equals(target.getMonth())
                && this.getDay().equals(target.getDay()));
    }
    public boolean isBefore(HtDateTime target) {
        if (!this.zone.equals(target.zone)) throw new IllegalArgumentException("HtDateTime.isBefore(HtDateTime target) - Can't compare time with different timezone! this.zone: '"+this.zone+"', target.zone: '"+target.zone+"'");

        return this.localDateTime.isBefore(target.localDateTime);
    }
    public boolean isAfter(HtDateTime target) {
        if (!this.zone.equals(target.zone)) throw new IllegalArgumentException("HtDateTime.isAfter(HtDateTime target) - Can't compare time with different timezone! this.zone: '"+this.zone+"', target.zone: '"+target.zone+"'");

        return this.localDateTime.isAfter(target.localDateTime);
    }

    public boolean isBetween(HtDateTime start, HtDateTime end) {
        return this.isAfter(start) && this.isBefore(end);
    }

    public HtDateTime setNano(int nano) {
        return set(NANO_OF_SECOND, nano);
    }
    public HtDateTime setSecond(int second) {
        return set(SECOND_OF_MINUTE, second);
    }
    public HtDateTime setMinute(int minute) {
        return set(MINUTE_OF_HOUR, minute);
    }
    public HtDateTime setHour(int hour) {
        return set(HOUR_OF_DAY, hour);
    }

    public HtDateTime setDay(int day) {
        return set(DAY_OF_MONTH, day);
    }
    public HtDateTime setMonth(int month) {
        return set(MONTH_OF_YEAR, month);
    }
    public HtDateTime setYear(int year) {
        return set(YEAR, year);
    }

    public HtDateTime setDayOfWeek(DayOfWeek dayOfWeek) {
    ...
    public HtDateTime setDayToLastOfMonth() {
        // 현재 날짜 기준으로 이달의 마지막 날로 설정
        return set(TemporalAdjusters.lastDayOfMonth());
    }


    public HtDateTime addYears(int amount) {
        return add(amount, YEARS);
    }
    public HtDateTime addMonths(int amount) {
        return add(amount, MONTHS);
    }
    public HtDateTime addWeeks(int amount) {
        return add(amount, WEEKS);
    }
    public HtDateTime addDays(int amount) {
        return add(amount, DAYS);
    }
    public HtDateTime addHours(int amount) {
        return add(amount, HOURS);
    }
    public HtDateTime addMinutes(int amount) {
        return add(amount, MINUTES);
    }
    public HtDateTime addSeconds(int amount) {
        return add(amount, SECONDS);
    }


    public HtDateTime subtractYears(int amount) {
        return subtract(amount, YEARS);
    }
    public HtDateTime subtractMonths(int amount) {
        return subtract(amount, MONTHS);
    }
    public HtDateTime subtractWeeks(int amount) {
        return subtract(amount, WEEKS);
    }
    public HtDateTime subtractDays(int amount) {
        return subtract(amount, DAYS);
    }
    public HtDateTime subtractHours(int amount) {
        return subtract(amount, HOURS);
    }
    public HtDateTime subtractMinutes(int amount) {
        return subtract(amount, MINUTES);
    }
    public HtDateTime subtractSeconds(int amount) {
     ...
     */
    public HtDateTime convertTo(ZoneId zoneTo) {
    ...
    public HtDateTime changeZone(ZoneId zoneTo) {
        setZone(zoneTo);
        return this;
    }
     ...
     */
    @Override
    public int compareTo(HtDateTime another) {
    ...
    public boolean equals(Object object) {
        if (object instanceof HtDateTime) {
            HtDateTime another = (HtDateTime) object;
