...
import com.radar.core.exception.RadarErrorType;
...
public class ObjectUtils {
    ...
    public static <T extends Collection<?>> T isResultEmpty(T result, RadarErrorType type) throws RadarCommonException {
        if (isEmpty(result)) {
            throw new RadarCommonException(type, ERROR_READ, "No Result (" + result.getClass().getName() + ") !!!");
        } else {
            return result;
        }
    }

    public static <T extends Map<?, ?>> T isResultEmpty(T result, RadarErrorType type) throws RadarCommonException {
        if (isEmpty(result)) {
            throw new RadarCommonException(type, ERROR_READ, "No Result (" + result.getClass().getName() + ") !!!");
        } else {
            return result;
    ...
    public static <T> T isResultEmpty(T result, RadarErrorType type) throws RadarCommonException {
        if (result == null) {
            throw new RadarCommonException(type, ERROR_READ, "No Result () !!!");
        } else {
            return result;
        }
    }
