...
import static com.radar.core.util.HtDateTimeUtil.*;

public class RadarCommonException extends NestedRuntimeException {
    /**
    ...
    @Getter
    private RadarErrorType type = ERROR_SYSTEM;
    @Getter
    private RadarServiceStatusCode reason = ERROR_SYSTEM_EXCEPTION;

    private String message;

    // 정보를 노출하기 위한 목적이 아닌 "예외를 던진다"라는 목적에 맞춘 생성자
    public RadarCommonException() {
        super("");
        this.message = "";
    }

    // 간단한 정보만을 노출시키기 위한 생성자 (메세지를 그대로 출력)
    public RadarCommonException(String message) {
        super(message);
    ...
    public RadarCommonException(RadarErrorType type, RadarServiceStatusCode reason, String message) {
        super(message);
    ...
    public RadarCommonException(ResultInfo resultInfo) {
    ...
    public RadarCommonException(Throwable throwable) {
    ...
    // 자세한 정보를 실어나르기 위한 생성자
    public RadarCommonException(RadarErrorType type, RadarServiceStatusCode reason, Throwable throwable) {
        super(throwable.getMessage(), throwable);
    ...
    // 에러코드 set
    private void setErrorCode(RadarErrorType type, RadarServiceStatusCode reason) {
    ...
    private void setErrorCode(Throwable throwable) {
        ...
        } else if (throwable instanceof RadarCommonException) {
            RadarErrorType type = getTypeByMessage(throwable.getMessage());
            RadarServiceStatusCode reason = getReasonByMessage(throwable.getMessage());
    ...
    private void setMessage(String message) {
        this.message = "[" + getNow(KST).get(ISO_DATE_TIME) + "] " + this.type.getName() + this.reason.getError() + " " + this.reason.getReason() + ": ";
        ...
        if (cause != null) {
            ...
            if (cause instanceof NullPointerException) {
                for (StackTraceElement element : cause.getStackTrace()) {
                    if (message == null && element.getClassName().startsWith("com.hanteo")) {
    ...
    private static RadarErrorType getTypeByMessage(String message) {
        Matcher matcher = Pattern.compile("(HT_[A-Z_]+)[0-9]+").matcher(message);
        if (matcher.find()) {
            String typeName = matcher.group(1);
            for (RadarErrorType type : RadarErrorType.values())
                if (ObjectUtils.nullSafeEquals(type.getName(), typeName))
                    return type;
        }
        return ERROR_SYSTEM;
    }
    private static RadarServiceStatusCode getReasonByMessage(String message) {
        Matcher matcher = Pattern.compile("HT_[A-Z_]+([0-9]+)").matcher(message);
        if (matcher.find()) {
            int reasonError = Integer.parseInt(matcher.group(1));
            for (RadarServiceStatusCode reason : RadarServiceStatusCode.values())
