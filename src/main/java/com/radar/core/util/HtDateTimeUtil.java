...
import static com.radar.core.exception.RadarServiceStatusCode.ERROR_PARAM_VALIDITY;
...
public class HtDateTimeUtil {
    ...
    public static int getDaysBetween(HtDateTime start, HtDateTime end) {
    ...
    public static int getDateTimeBetween(ChronoUnit unit, HtDateTime start, HtDateTime end) {
    ...
    public static boolean isEqual(HtDateTime a, HtDateTime b) {
        return (a == null && b == null) || (a != null && a.isEqual(b));
    }

    public static boolean isSameOnFormat(String pattern, HtDateTime a, HtDateTime b) {
    ...
    public static boolean isSameDate(HtDateTime a, HtDateTime b) {
    ...
    /*
     * HtDateTime 객체를 좀 더 편하게 생성할 수 있도록 도와주는 유틸리티 메서드
     */
    public static HtDateTime getDateTimeFrom(ZoneId zone, String input) {
        LocalDateTime localDateTime = createLocalDateTime(zone, input);

        if (localDateTime != null) {
            HtDateTime.Builder builder = new HtDateTime.Builder();
    ...
    public static HtDateTime getDateTimeFrom(ZoneId zone, String input, String format) throws RadarCommonException {
        ...
        try {
            ...
            throw new RadarCommonException(ERROR_SYSTEM, ERROR_PARAM_VALIDITY, e);
        }
    ...
    public static HtDateTime getDateTimeFrom(ZoneId zone, long epochMillis) {
    ...
    public static HtDateTime getDateTimeFromUTC(String input) {
        return getDateTimeFrom(UTC, input);
    }

    public static HtDateTime getDateTimeFromUTC(String input, String format) throws RadarCommonException {
        return getDateTimeFrom(UTC, input, format);
    }

    public static HtDateTime getDateTimeFromUTC(long epochMillis) {
        return getDateTimeFrom(UTC, epochMillis);
    }
    
    public static HtDateTime getNow(ZoneId zone) {
        HtDateTime.Builder builder = new HtDateTime.Builder();
        return builder.zone(zone).localDateTime(ZonedDateTime.now(zone).toLocalDateTime()).build();
    }
    public static HtDateTime getNowUTC() {
        return getNow(UTC);
    }
    ...
    private static LocalDateTime createLocalDateTime(ZoneId zone, String input) {
        ...
        if (hasDate) {
            ...
            if ("0000".equals(dateMatcher.group("year"))) {
                ...
                logger.warn("Input HtDateTime '{}' has zero year! returning 'NULL' (If it's not birthday, please check HtDateTimeUtil.createLocalDateTime())", input);
                return null;
            } else {
                ...
                if (year < 100) {
                    ...
                    logger.warn("Input HtDateTime '{}' has 2 digits year! converting year '{}' to '{}' (If it's unexpected, please check HtDateTimeUtil.createLocalDateTime())", input, yearBeforeProcess, year);
                }
            }
            ...
            if (month == 0 || day == 0) {
                ...
                if (month == 0) month = 1;
                if (day == 0) day = 1;
                logger.warn("Input HtDateTime '{}' has zero month or zero day! returning '{}-{}-{}' (If it's not birthday, please check HtDateTimeUtil.createLocalDateTime())", input, year, month, day);
        ...
        if (date == null && time == null) throw new RadarCommonException(ERROR_SYSTEM, ERROR_PARAM_VALIDITY,
    ...
    private static LocalTime createLocalTime(String input) {
        ...
        if (hour < 0 || hour >= 24 || minute < 0 || minute >= 60 || second < 0 || second >= 60 || nano < 0 || nano >= 1000000000) {
            throw new RadarCommonException(ERROR_SYSTEM, ERROR_PARAM_VALIDITY, "Wrong time(hour="+hour+",minute="+minute+",second="+second+",nano="+nano+"), please check your input");
        } else {
            return LocalTime.of(hour, minute, second, nano);
        }
    }

    public static class HtDateSerializer extends JsonSerializer<HtDateTime> {
        @Override
        public void serialize(HtDateTime htDateTime, JsonGenerator generator, SerializerProvider provider) throws IOException {
            generator.writeString(htDateTime.get(ISO_DATE));
        }
    }

    public static class HtDateTimeSerializer extends JsonSerializer<HtDateTime> {
        @Override
        public void serialize(HtDateTime htDateTime, JsonGenerator generator, SerializerProvider provider) throws IOException {
            generator.writeString(htDateTime.get(ISO_DATE_TIME));
        }
    }

    public static class HtDateTimeDeserializer extends JsonDeserializer<HtDateTime> {
        @Override
        public HtDateTime deserialize(JsonParser parser, DeserializationContext context) throws IOException, JsonProcessingException {
            ...
            try {
                ...
                throw new RuntimeException(MessageFormat.format("Wrong HtDateTime input, please check parameter (input text: {0})", isoDateTime));
            }
        }
    }
